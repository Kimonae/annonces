...
$sql ="SELECT id, titre, photo, description, categorie, prix, datepublication, lieu FROM produits WHERE id = :id";
$query = $db->prepare($sql);
$query->bindValue(":id", $id, PDO::PARAM_INT);
$query->execute();
...
echo "<h5> Si vous modifiez une annonce, les informations écrasées seront perdues dans le néant </h5>";

echo "<div id='drop'>";
echo '<a href="drop.php?id='.$d['id'] .'"> <input type="button" id="drop" value ="Supprimer !"> </a>' ; 
echo "</div>";
}
}
